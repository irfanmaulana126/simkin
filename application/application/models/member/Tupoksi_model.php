...
class tupoksi_model extends CI_Model
{
    ...
    public function datasDetailTupoksiTindakan($usr_id,$tupoksi,$date,$unit_kerja)
    {
        $query =$this->db->query("
        select f.id_usr,fol_pelaksana_tipe,fol_pelaksana_nominal,fol_id,id_reg,fol_nama,cust_usr_nama,fol_nominal,tindakan_tanggal,tindakan_waktu,usr_id,indikator,difinisi,jns_input from simkin.master_indikator as a 
                    ...
                    and tindakan_tanggal::TEXT like '".$date."%' and usr_id='".$usr_id."' and a.id='".$tupoksi."' and a.id_unit_kerja='".$unit_kerja."' GROUP BY tindakan_tanggal,f.id_usr,fol_pelaksana_tipe,fol_pelaksana_nominal,fol_id,id_reg,fol_nama,cust_usr_nama,fol_nominal,tindakan_waktu,usr_id,indikator,difinisi,jns_input ORDER BY tindakan_tanggal DESC");
    ...
    public function datasDetailTupoksiFolio($usr_id,$tupoksi,$date,$unit_kerja)
    {
        $query =$this->db->query("
        ...
        INNER JOIN hris.hris_pegawai as g on f.id_pgw=g.pgw_id WHERE fol_pelaksana_tipe IN('1','10')
                    and tindakan_tanggal::TEXT like '".$date."%' and usr_id='".$usr_id."' and a.id='".$tupoksi."' and a.id_unit_kerja='".$unit_kerja."' GROUP BY tindakan_tanggal,d.id_usr,fol_pelaksana_tipe,fol_pelaksana_nominal,fol_id,id_reg,fol_nama,cust_usr_nama,fol_nominal,tindakan_waktu,usr_id,indikator,difinisi,jns_input  ORDER BY tindakan_tanggal DESC");
        $indikator = $query->result();
        if(!empty($indikator)){
            return $indikator;
         } else {
...
(select a.id,f.id_usr as usr_id,target,bobot from simkin.master_indikator as a 
                    ...
                    INNER JOIN hris.hris_pegawai as i on g.id_pgw=i.pgw_id 
                                    INNER JOIN target_bobot as j on j.id_m_indikator=b.id_master_indikator
                                    WHERE fol_pelaksana_tipe IN('1','10') and tindakan_tanggal::TEXT like '".$date."%' and f.id_usr='".$this->session->userdata('userId')."' and tgl_akhir::TEXT<='".$date."%'  GROUP BY a.id,f.id_usr,target,bobot)aa on b.id=aa.id
    UNION
...
(select a.id,d.id_usr as usr_id,target,bobot from simkin.master_indikator as a 
                    ...
                    INNER JOIN hris.hris_pegawai as g on f.id_pgw=g.pgw_id
                                    ...
                                    WHERE fol_pelaksana_tipe IN('1','10') and tindakan_tanggal::TEXT like '".$date."%' and d.id_usr='".$this->session->userdata('userId')."' and tgl_akhir::TEXT<='".$date."%' GROUP BY a.id,d.id_usr,target,bobot)bb on d.id=bb.id
                                            ) as b
