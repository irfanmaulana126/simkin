...
class pegawai_unit extends CI_Model
{
    ...
    {
        $query =$this->db->query("
        SELECT  a.id,indikator,difinisi,created_at,updated_at,usr_insrt,usr_edit,indikator_tupoksi,id_unit_kerja,aktif,id_jabatan,jns_input,input,sumall,target,bobot, sumall / target * bobot as total FROM simkin.master_indikator as a LEFT JOIN 
        (
        SELECT id as input,id_master_indikator as id ,usr_id,sum(nilai) as sumall,target,bobot from input_kegitan_tupoksi WHERE usr_id='".$id."' and created::TEXT like '".$date."%' and aktif='Y' GROUP BY id,id_master_indikator,usr_id,usr_insrt,aktif,target,bobot
        UNION
        select '0'as input,a.id,f.id_usr as usr_id,count(cust_usr_nama) as sumall,target,bobot from simkin.master_indikator as a 
                    ...
                    INNER JOIN hris.hris_pegawai as i on g.id_pgw=i.pgw_id 
                                    ...
                                    WHERE fol_pelaksana_tipe IN('1','10') and tindakan_tanggal::TEXT like '".$date."%' and f.id_usr='".$id."' and tgl_akhir::TEXT<='".$date."%'  GROUP BY a.id,f.id_usr,target,bobot
    UNION
    select '0'as input,a.id,d.id_usr as usr_id,count(cust_usr_nama) as sumall,target,bobot from simkin.master_indikator as a 
                    ...
                    INNER JOIN hris.hris_pegawai as g on f.id_pgw=g.pgw_id
                                    ...
                                    WHERE fol_pelaksana_tipe IN('1','10') and tindakan_tanggal::TEXT like '".$date."%' and d.id_usr='".$id."' and tgl_akhir::TEXT<='".$date."%' GROUP BY a.id,d.id_usr,target,bobot
                                            ) as b
