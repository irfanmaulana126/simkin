...
class tupoksi_model extends CI_Model
{
    ...
    public function datasTupoksiKuantitasPegawai($date)
    {
        ...
        UNION
        (select id_tindakan,id_jenis_pos,id_pos,kategori_tindakan_nama as indikator,target,bobot,definisi_ops as difinisi_ops,count(kategori_tindakan_id) as nilai,((count(kategori_tindakan_id)::FLOAT / target) * bobot) as tot,'dokter' as pegawai from klinik.klinik_folio_pelaksana a 
             INNER JOIN klinik.klinik_folio b on b.fol_id = a.id_fol 
						 ...
						 GROUP BY id_tindakan,kategori_tindakan_nama,nama_unit,definisi_ops,id_jenis_pos,id_pos,target,bobot)
        ");
    ...
    public function datasDetailTupoksiKuantitasPegawai($tupoksi,$date)
    {
        ...
        INNER JOIN klinik.klinik_folio b on b.fol_id = a.id_fol 
                    ...
                    where tindakan_tanggal::TEXT like '".$date."%' and usr_id='".$this->session->userdata('userId')."' and g.id_tindakan='".$tupoksi."' and g.id_pos='".$this->session->userdata('unit')."' 
		");
        $indikator = $query->result();
        if(!empty($indikator)){
            return $indikator;
